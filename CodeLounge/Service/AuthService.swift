...
enum ServiceError: Error {
  case firebaseInvalidated
  
  case googleClientIDError
...
final class AuthService: AuthServiceProtocol {
  ...
  func handleSignInWithAppleCompletion(
    ...
    return Future { [weak self] promise in
        self?.handleSignInWithAppleCompletion(
          authorization,
          nonce: nonce
        ) { result in
            switch result {
...
private extension AuthService {
  ...
  func signInWithGoogle(
    ...
    guard let clientID = FirebaseApp.app()?.options.clientID else {
        completion(.failure(ServiceError.googleClientIDError)) // 실패시
        return
    }
    ...
    GIDSignIn.sharedInstance.signIn(withPresenting: rootViewController) { [weak self] result, error in
        ...
        guard let user = result?.user, let idToken = user.idToken?.tokenString else {
            // 유저정보, 토큰정보가 없다면
          completion(.failure(ServiceError.googleTokenError))
            return
        }
      ...
      self?.authenticateUserWithFirebase(
  ...
  private func handleSignInWithAppleCompletion(
    ...
    guard let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential,
      ...
      completion(.failure(ServiceError.appleTokenError))
      return
    }
  ...
  private func authenticateUserWithFirebase(
    credential: AuthCredential, loginPlatform: LoginPlatform,
    completion: @escaping (Result<User, Error>) -> Void
  ) {
    ...
    Auth.auth().signIn(with: credential) { result, error in
      ...
      guard let result else {
        completion(.failure(ServiceError.firebaseInvalidated))
        return
      }
